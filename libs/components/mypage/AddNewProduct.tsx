...
// Extend ProductInput to include productImages if not already present
type ProductInputWithImages = ProductInput & { productImages: string[] };
...
const AddProduct = ({ initialValues, ...props }: any) => {
	...
	const [insertProductData, setInsertProductData] = useState<ProductInputWithImages>(initialValues);
	...
	const insertProductHandler = useCallback(async () => {
		try {
			...
			await toastSuccess('This product has been created successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myProducts',
	...
	const updateProductHandler = useCallback(async () => {
		try {
			...
			await toastSuccess('This product has been updated successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myProducts',
