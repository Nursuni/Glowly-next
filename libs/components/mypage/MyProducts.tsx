...
const MyProducts: NextPage = ({ initialInput, ...props }: any) => {
	...
	const deleteProductHandler = async (id: string) => {
		try {
			if (await toastWarning('are you sure to delete this product?')) {
				await updateProduct({
					variables: {
						input: {
							_id: id,
	...
	const updateProductHandler = async (status: string, id: string) => {
		try {
			if (await toastWarning(`are you sure change to ${status} status?`)) {
				await updateProduct({
					variables: {
						input: {
							_id: id,
	...
	if (device === 'mobile') {
		return <div>GLOWLY PRODUCTS MOBILE</div>;
	} else {
		return (
